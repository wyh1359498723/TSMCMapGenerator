...
namespace TSMCMapGenerator.Services
{
    public class TsmcMapService
    {
        ...
        public async Task Generate(string cust, string device, string rpForCurrentWafer, List<Stdf_BinsGroupModel> wafers)
        {
            try
            {
                Log.Information("[INFO] {Cust}-{Device} 在 MMS_TSMC_DEVICE 表中，开始为 LotId: {LotId}, CP: {Cp} 生成 TSMC Map 文件...", cust, device, wafers[0].LotId, wafers[0].CP);

                await CreateTsmcMap(cust, device, rpForCurrentWafer, wafers[0]);
            }
            catch (Exception ex)
            {
                Log.Error(ex, "生成 TSMC Map 文件失败。客户: {Cust}, 设备: {Device}, LotId: {LotId}, CP: {Cp}", cust, device, wafers[0].LotId, wafers[0].CP);
            }
        }
        ...
        private async Task CreateTsmcMap(string cust, string device, string rpForCurrentWafer, Stdf_BinsGroupModel wafer)
        {
            try
            {
                ...
                {
                    Log.Warning("未找到 LotId: {LotId}, Wf_No: {WfNo} 的 Split 信息，跳过生成 TSMC Map。", wafer.LotId, wafer.Wf_No);
                    return;
                }
                ...
                {
                    Log.Warning("未找到 LotId: {LotId}, CP: {Cp} 的 Head 信息，跳过生成 TSMC Map。", wafer.LotId, wafer.CP);
                    return;
                }
                ...
                {
                    Log.Warning("未找到 LotId: {LotId}, CP: {Cp}, Wf_No: {WfNo} 的 P_DATA_DETAIL 信息，跳过生成 TSMC Map。", wafer.LotId, wafer.CP, wafer.Wf_No);
                    return;
                }
                ...
                {
                    Log.Warning("无法获取 {LotId}-{WfNo} 的任何坐标数据，跳过生成 TSMC Map。", wafer.LotId, wafer.Wf_No);
                    return;
                }
                ...
                if (wafer.CP.StartsWith("CP") && int.TryParse(wafer.CP.Substring(2), out currentCpNumber))
                {
                    for (int i = 1; i < currentCpNumber; i++)
                    {
                        ...
                        {
                            Log.Information("[INFO]   正在获取历史晶圆数据 LotId: {LotId}, CP: {Cp}, RP: {Rp}, Wfno: {WfNo}", wafer.LotId, previousCpString, latestRpForHistoricalCp, wafer.Wf_No);
                            ...
                            {
                                Log.Warning("未能从 API 获取到历史晶圆数据或数据为空。LotId: {LotId}, CP: {Cp}, RP: {Rp}, Wfno: {WfNo}", wafer.LotId, previousCpString, latestRpForHistoricalCp, wafer.Wf_No);
                            }
                        }
                        else
                        {
                            Log.Information("未找到 LotId: {LotId}, CP: {Cp} 的最新 RP，跳过获取历史晶圆数据。", wafer.LotId, previousCpString);
                        }
                    }
                }
